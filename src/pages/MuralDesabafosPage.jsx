...
const MuralDesabafosPage = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-rose-50 via-pink-50 to-purple-50">
      ...
      <main className="px-4 py-6 pb-32 max-w-lg mx-auto">
        ...
        {activeTab === 'new' && (
          ...
          >
            ...
            {/* Toggle Carta */}
            <div className="flex items-center justify-between bg-white rounded-2xl p-4 shadow-sm border border-pink-100">
              <div className="flex items-center gap-3">
                <span className="text-2xl">💌</span>
                <div>
                  <p className="font-medium text-gray-800">Modo Carta</p>
                  <p className="text-xs text-gray-500">Escreva como uma carta romântica</p>
                </div>
              </div>
              <button
              ...
              >
                <span className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow transition-transform ${
                  isLetter ? 'translate-x-6' : 'translate-x-1'
                }`}/>
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className={`bg-white rounded-2xl shadow-sm border overflow-hidden ${
                ...
                <textarea
                  ...
                  placeholder={isLetter ? "Querido Pablo,\n\nEscreva sua carta de amor aqui..." : "Desabafa aqui, princesa... 💭"}
      ...
      {/* Botão Escondido - Acesso Admin (quase invisível) */}
      {!isAdmin && (
        <button
          onClick={() => navigate('/admin')}
          className="fixed bottom-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-pink-200/30 rounded-full hover:bg-pink-300/50 transition-colors cursor-default"
          title=""
        />
      )}
    </div>
